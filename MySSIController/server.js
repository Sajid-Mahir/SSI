    ...
    .then((resp) => {
      ...
      if (resp) {
        ...
        try {
          ...
          // console.log('User email Memoname Connection ID saved successfully: ', response)
        } catch (error) {
          if (error.code === 11000) {
            // duplicate key
            return res.json({
...
app.post("/credential", async function (req, res) {
  ...
  try {
    const resp = await axios.get(
      "http://127.0.0.1:8021/credential-definitions/created"
    );
...
app.get("/proofReq", async function (req, res) {
  ...
  const role = req.session.reqPage === "Page1" ? "student" : "faculty";
  ...
  if (req.cookies.conID) {
    ...
    try {
      const temp = await axios.post(
        "http://127.0.0.1:8021/present-proof/send-request",
        data
      );
	  console.log("present-proof response-");
	  console.log(temp.data);
	  
      console.log("waiting for response from holder...");
...
app.post("/twoFactor", function (req, res) {
  ...
  console.log("At twoFactor with:", conID, " and Memorable Name:", memoName);

  // const user = twoFactor.findOne({ connectionID })
  ...
  twoFactor.findOne({ connectionID: connectionID }, function (err, obj) {
    ...
    //console.log("Retrieved from database:", obj.email, "-", obj.memoName, "-", obj.connectionID)
  ...
  // console.log("Retrieved from database:", user.email, "-", user.memoName, "-", user.connectionID)
...
app.post("/twoFactorCheck", function (req, res) {
  // Your code here
  console.log("At two factor check!");
  ...
  if (req.cookies.conID && req.cookies.memoName) {
    ...
    console.log(
      "At TwoFactorCheck: Connection ID:",
      connectionID,
      " and MemoName:",
      memoName
    );

    // twoFactorData.findOneAndUpdate({connectionID: connectionID, memoName: memoName, code: codeData}, {$set:{status:true}}, {new: true}, (err, doc) => {
    // 	if (err) {
    ...
    // twoFactor.findOne({connectionID: connectionID, memoName: memoName, code: codeData}, function(err,obj) {
...
app.post("/activeConctd", function (req, res) {
  ...
  console.log("POST /activeConctd connection_id:", conID);
  res.cookie("conID", conID, { maxAge: 900000, httpOnly: true });
  req.session.conID = conID;
  res.render("proof");
});

app.post("/checkDID", async (req, res) => {
    const { did } = req.body;
    console.log("At checkMemName with:", did);

    try {
        const resp = await axios.get("http://127.0.0.1:8021/connections");
        const connection = resp.data["results"].find(element => element.their_did?.includes(did));

        res.setHeader("Content-Type", "application/json");
        if (connection) {
            console.log("Matching found....");
            res.end(JSON.stringify({ status: false, connection: connection.connection_id }));
        } else {
            res.end(JSON.stringify({ status: true }));
        }
    } catch (err) {
...
app.get("/generate", function (req, res) {
  ...
  var returnData;
  axios
    .post("http://127.0.0.1:8021/connections/create-invitation", data)
    .then((resp) => {
      // console.log(`Status: ${res.status}`);
      // console.log('Body: ', res.data);
...
function checkUserSession2(req, res, next) {
  ...
  if (req.session.username) {
    ...
    if (
      req.session.retrievedAttribute &&
      req.session.retrievedAttribute === "faculty"
    )
      res.render("page2");
